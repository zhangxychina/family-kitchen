...
3. 添加 iCloud capability，勾选 CloudKit，创建或选择该 Team 的容器。默认占位配置是 `iCloud.com.familykitchen.app`，并不表示该容器已在你的账号下创建。
...
本机目前只有 Command Line Tools，没有完整 Xcode/iOS SDK。已验证核心测试、CloudKit 控制器在 macOS SDK 下的类型检查、SwiftUI 源码语法和 plist；不等于完整 iOS 构建或真实云端验证通过。
...
- 重启后先验证原 iCloud 身份，再展示共享缓存；离线冷启动可能停留在共享状态页。运行中断网可以继续编辑已验证的缓存。账号变更或检测到权限撤销时隐藏共享厨房并停止编辑，不把旧数据上传到新账号。
